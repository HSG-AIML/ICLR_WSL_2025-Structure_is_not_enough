...
class AEModule(nn.Module):
    def __init__(self, config, reference_checkpoint=None, reference_params=None):
        ...
        self.criterion = GammaContrastReconLoss(
            ...
            beta=config.get("training::beta", 0.1),
            loss_distillation=config.get("training::loss_distillation", "l2"),
            temperature_distillation=config.get("training::temperature_distillation", 2.0),
            queryset_distillation=config.get("training::queryset_distillation", "random"),
            queryset_dump=config.get("training::queryset_dump", None),
            n_queries_distillation=config.get("training::n_queries_distillation", 64),
            reference_checkpoint=reference_checkpoint,
            reference_params=reference_params,
            lambda_=config.get("training::lambda", 0.0),
        )
    ...
    def train_step(self, x_i, m_i, p_i, x_j, m_j, p_j):
        ...
        with torch.cuda.amp.autocast(enabled=True):
            ...
            z_hat_i = self.forward_encoder(y_i, p_i)

            z_j, y_j, zp_j = self.forward(x_j, p_j)
            z_hat_j = self.forward_encoder(y_j, p_j)
            ...
            z_hat = torch.cat([z_hat_i, z_hat_j], dim=0)

            logging.debug(
                f"train step - x: {x.shape}; y: {y.shape}, m: {m.shape}, z_i {z_i.shape}; z_j {z_j.shape};  zp_i {zp_i.shape}; zp_j {zp_j.shape}"
            )
            # compute loss
            perf = self.criterion(z_i=zp_i, z_j=zp_j, y=y, t=x, m=m, p=p, z=z, z_hat=z_hat)
