...
class MLP(nn.Module):
    ...
    def forward_with_state_dict(self, x, state_dict):
        # forward prop through module_list
        x = x.float()
        for idx, layer in enumerate(self.module_list):
            if isinstance(layer, nn.Conv2d):
                x = F.conv2d(x, state_dict[f"module_list.{idx}.weight"].float(), bias=state_dict[f"module_list.{idx}.bias"].float(), stride=layer.stride, padding=layer.padding, dilation=layer.dilation, groups=layer.groups)
            elif isinstance(layer, nn.Linear):
                x = F.linear(x, state_dict[f"module_list.{idx}.weight"].float(), bias=state_dict[f"module_list.{idx}.bias"].float())
            else:
                x = layer(x)
        return x
...
class CNN(nn.Module):
    ...
    def forward_with_state_dict(self, x, state_dict):
        ...
        for idx, layer in enumerate(self.module_list):
            if isinstance(layer, nn.Conv2d):
                x = F.conv2d(x, state_dict[f"module_list.{idx}.weight"].type(x.dtype), bias=state_dict[f"module_list.{idx}.bias"].type(x.dtype), stride=layer.stride, padding=layer.padding, dilation=layer.dilation, groups=layer.groups)
            elif isinstance(layer, nn.Linear):
                x = F.linear(x, state_dict[f"module_list.{idx}.weight"].type(x.dtype), bias=state_dict[f"module_list.{idx}.bias"].type(x.dtype))
            else:
                x = layer(x)
        return x
...
class LeNet5(nn.Module):
    def __init__(
        ...
        # Layer 0
        self.module_list.append(nn.Conv2d(channels_in, 6, kernel_size=5))
        self.module_list.append(self.get_nonlin(nlin))
        self.module_list.append(nn.MaxPool2d(kernel_size=2, stride=2))
        self.module_list.append(nn.Dropout(dropout))

        # Layer 1
        self.module_list.append(nn.Conv2d(6, 16, kernel_size=5))
        self.module_list.append(self.get_nonlin(nlin))
        self.module_list.append(nn.MaxPool2d(kernel_size=2, stride=2))
        self.module_list.append(nn.Dropout(dropout))
        ...
        self.module_list.append(nn.Linear(16 * 5 * 5, 120))
        self.module_list.append(self.get_nonlin(nlin))
        self.module_list.append(nn.Dropout(dropout))

        # Fully connected layer 2
        self.module_list.append(nn.Linear(120, 84))
        self.module_list.append(self.get_nonlin(nlin))
        self.module_list.append(nn.Dropout(dropout))

        # Output layer
        self.module_list.append(nn.Linear(84, 10))
...
class NNmodule(nn.Module):
    def __init__(self, config, cuda=False, seed=42, verbosity=0):
        ...
        elif config["model::type"] == "LeNet5":
            # calling MLP constructor
            logging.info("=> creating model LeNet5")
            model = LeNet5(
                channels_in=config["model::channels_in"],
                nlin=config["model::nlin"],
                dropout=config["model::dropout"],
