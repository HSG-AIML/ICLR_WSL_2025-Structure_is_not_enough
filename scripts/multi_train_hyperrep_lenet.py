...
from shrp.git_re_basin.git_re_basin import (
    zoo_lenet_permutation_spec
)


PATH_ROOT = Path("/netscratch2/lmeynent/research/structure_vs_behaviour/results/hyperrepresentations")
PATH_ZOO = Path("/netscratch2/lmeynent/research/structure_vs_behaviour/results/zoos/multi_train_zoo_lenet")
...
def main():
    ...
    config["ae:i_dim"] = 256
    config["ae:lat_dim"] = tune.grid_search([32, 64, 128])
    # config["ae:lat_dim"] = 64
    # 9609,   40,  511
    config["ae:max_positions"] = [1024, 64, 128]
    ...
    # AMP
    #
    config["training::epochs_train"] = 256
    ...
    config["training::beta"] = tune.grid_search([0.1, 1.])
    config["training::loss_distillation"] = 'l2'
    config["training::temperature_distillation"] = 2.0
    config["training::queryset_distillation"] = "data"
    config["training::queryset_dump"] = "/netscratch2/lmeynent/research/structure_vs_behaviour/results/data/multi_zoo/dataset.pt"
    config["training::n_queries_distillation"] = tune.grid_search([256, 1024])
    ...
    if not Path(config["training::queryset_dump"]).is_file():
        Path(config["training::queryset_dump"]).parent.mkdir(parents=True, exist_ok=True)

        query_set_dumps = "/netscratch2/lmeynent/research/structure_vs_behaviour/results/data/{}_train_zoo_relu/dataset.pt"
        query_sets_raw = [torch.load(query_set_dumps.format(ds))['trainset'] for ds in ['svhn', 'cifar10', 'eurosat']]

        len_qs = np.min([len(qs) for qs in query_sets_raw])
        
        query_sets = list()
        for query_set in query_sets_raw:
            if len(query_set) > len_qs:
                labels = [sample[1] for sample in query_set]
                idx_selected = StratifiedShuffleSplit(train_size=len_qs, random_state=2020).split(np.zeros(len(labels)), labels)
                query_sets.append(torch.utils.data.Subset(query_set, next(idx_selected)[0]))
            else:
                query_sets.append(query_set)

        query_set = {'trainset': torch.utils.data.ConcatDataset(query_sets)}

        torch.save(query_set, config["training::queryset_dump"])
    ...
    if not (data_path / "dataset_beton.train").is_file() or not (data_path / "dataset_beton.test").is_file():
...
def prep_data(target_dataset_path, config, property_keys):
    ...
    permutation_spec = zoo_lenet_permutation_spec()
