...
PATH_ZOO = Path("/netscratch2/lmeynent/research/structure_vs_behaviour/results/zoos/svhn_train_zoo")
...
def main():
    ...
    # permutation specs
    config["training::permutation_number"] = 10
    config["training::view_1_canon"] = True
    # config["training::view_2_canon"] = tune.grid_search([False, True])
    # config["training::view_2_canon"] = False
    # config["training::permutations_per_sample"] = 10
    config["testing::permutation_number"] = 10
    ...
    config["training::gamma"] = tune.grid_search([0., 0.025, 0.05, 0.1])
    config["training::reduction"] = "mean"
    config["training::contrast"] = "simclr"
    # AMP
    #
    config["training::epochs_train"] = 150
    ...
    config["training::recon"] = "both"
    config["training::beta"] = tune.grid_search([0., 0.05, 0.1, 0.25, 0.5])
    config["training::temperature_distillation"] = 2.0
    config["training::queryset_distillation"] = tune.grid_search(["random", "data", "union"])
    config["training::queryset_dump"] = "/netscratch2/lmeynent/research/structure_vs_behaviour/results/data/svhn_train_zoo/dataset.pt"
    config["training::n_queries_distillation"] = tune.grid_search([128, 256, 512, 1024])
...
def prep_data(target_dataset_path, config, property_keys):
    ...
    permutations_per_sample_train = min(5, permutation_number_train)
    permutation_number_test = config["testing::permutation_number"]
    permutations_per_sample_test = min(5, permutation_number_test)
