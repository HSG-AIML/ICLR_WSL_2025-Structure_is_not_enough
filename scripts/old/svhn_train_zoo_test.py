...
def main():
    ...
    config["model::init_type"] = tune.grid_search(["kaiming_uniform", "kaiming_normal"])
    config["optim::optimizer"] = "adam"
    config["optim::lr"] = tune.grid_search([1e-3, 1e-4])
    config["optim::wd"] = tune.grid_search([1e-3, 1e-4])

    config["seed"] = tune.grid_search(range(100, 120))
    ...
    try:
        ...
        trainset_raw = SVHN(
            root='/netscratch2/lmeynent/datasets/SVHN',
            split="train",
