...
def main():
    ...
    config["training::recon"] = tune.grid_search(["recon", "distillation", "both"])
    config["training::beta"] = 0.1
    config["training::temperature_distillation"] = 2.0
    config["training::queryset_distillation"] = tune.grid_search(["random", "data", "union"])
    config["training::queryset_dump"] = tune.grid_search([
        "/netscratch2/lmeynent/research/structure_vs_behaviour/results/data/svhn_train_zoo/dataset.pt",
        "/netscratch2/lmeynent/backdoored_zoos/data/CIFAR10/clean/dataset.pt"
    ])
